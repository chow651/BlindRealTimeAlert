...
int Inference::loadNcnnNetwork(AAssetManager* mgr, const char* modeltype , const int& modelInputShape, const float* meanVals, const float* normVals, bool useGpu)
{
    ...
    if (ret_param != 0)
    {
        __android_log_print(ANDROID_LOG_ERROR, "ncnn", "load_param failed ret=%d path=%s", ret_param, parampath);
        return ret_param;
    }
    ...
    if (ret_model != 0)
    {
        __android_log_print(ANDROID_LOG_ERROR, "ncnn", "load_model failed ret=%d path=%s", ret_model, modelpath);
        return ret_model;
    }

    netReady = true;
    __android_log_print(ANDROID_LOG_INFO, "ncnn", "loadNcnnNetwork success path=%s useGpu=%d", modelpath, useGpu ? 1 : 0);
...
std::vector<Object> Inference::runInference(const cv::Mat &bgr)
{
    ...
    std::vector<Object> proposals;
    proposals.reserve(1024);

    // stride 8
    {
        ncnn::Mat out;
        ex.extract("out0", out);
        generate_proposals(8, out, prob_threshold, proposals);
    }
    ...
    // stride 16
    {
        ncnn::Mat out;
        ex.extract("out1", out);
        generate_proposals(16, out, prob_threshold, proposals);
    }
    ...
    // stride 32
    {
        ncnn::Mat out;
        ex.extract("out2", out);
        generate_proposals(32, out, prob_threshold, proposals);
    }
